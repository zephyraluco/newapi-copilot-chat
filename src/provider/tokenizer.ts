 ...
 * Token 估算。
 *
 * VS Code 用它来判断「这段历史能不能塞进上下文」，用于决定是否需要裁剪。
 * 我们拿不到目标模型的真实分词器（各家不同，且网关也不暴露），因此只能估算。
 *
 * 估算策略：
 * - CJK 字符按 1 字符 ≈ 1 token（中文在主流分词器里通常就是这个量级）；
 * - 其余字符按 4 字符 ≈ 1 token；
 * - 每条消息、每个工具、每张图片加一份固定开销。
 *
 * **偏差方向很重要**：宁可高估。高估会让 VS Code 更早裁剪历史，代价是少一点上下文；
