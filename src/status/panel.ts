 ...
 * ## 与状态栏的分工
 *
 * 状态栏只有一格，只能回答「能不能用」。面板回答「为什么」：
 * 站点信息、配置问题、模型清单（含每个数值的来源）、会话用量、适配器链、日志入口。
 *
 * ## 渲染方式
 *
 * 面板只在打开时存在，但状态变化可能很频繁。因此：
 * - HTML 骨架只生成一次；状态通过 `postMessage` 增量下发，避免重建 DOM 导致滚动位置丢失；
 * - 模型清单可能上百条，不放进每次下发的 state 里，而是按需请求一次；
 * - Webview 脚本**全部用 DOM API 构造节点**（`textContent`），不拼 innerHTML——
...
(function () {
	...
	function renderAdapters(state) {
		...
		return h('div', {}, [
			items,
			h('div', { class: 'muted', text: '适配器用于处理不同上游的协议差异（当前均为占位实现）。' }),
		]);
	}
