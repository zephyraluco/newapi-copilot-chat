 ...
 * 状态服务。
 *
 * 它是「扩展当前处于什么状态」的唯一真相来源：有哪些配置组、各自是否可用、
 * 各自有多少模型、本次会话消耗了多少 token。
 *
 * 状态栏与面板都只做一件事——**订阅这里并渲染**，不各自去发请求。
 * 这样既避免了重复轮询，也保证两处显示永远一致。
 *
 * 配置可以有多个来源（VS Code 的多个配置组），因此状态是**按目标聚合**的：
 * 每个目标一行，整体可用性取「是否存在任一可用目标」。
 */
