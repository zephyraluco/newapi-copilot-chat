 ...
 *
 * New API 是 OpenAI 协议的网关，因此这里以 OpenAI Chat Completions 结构为基准，
 * 并额外标注了 DeepSeek、OpenRouter 等上游常见但**非** OpenAI 标准的扩展字段。
 *
 * 设计原则：
 * - 服务端字段一律可选（`?`）。网关版本、上游模型、代理层都可能裁掉字段，
 *   代码必须把「字段缺失」当常态而不是异常。
 * - 无法穷举的字段统一交给一个索引签名兜底，避免因为网关加了新字段就解析失败。
 */
