/**
 * 模型目录：编排「何时拉模型列表、原始数据怎么变成配置、失败了怎么办」。
 * 信息整合在 `modelConfig.ts`，网络交互在 `client`。
 *
 * 两个关键设计：
 * 1. **并发合并**：VS Code 可能在短时间内多次调用模型发现，用一个 in-flight promise 合并成一次请求。
 * 2. **失败保留旧数据**（stale-while-error）：刷新失败时返回上一次的快照并附上错误，
