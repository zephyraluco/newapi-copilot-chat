...
- **可解释的模型信息**：模型悬浮提示只列身份、规模与能力，且逐项一行、键值分列；
  每个数值的来源（模型数据表 / 网关返回值 / 默认值）与一致性校正的原因写进日志的 debug 级别。
...
- **状态栏**：显示连接状态与模型数量，鼠标悬停给出**本次会话的输入输出与缓存命中**
  （空闲时不弹，免得只看到一句「还没有请求」）；需要用户处理的问题（配置不完整、站点连不上）
  也会在悬浮提示里各占一行。
...
1. 站点没返回该信息而数据表又过时了，跑 `npm run models:openrouter` 重新生成；
2. 生成结果依然不对（上游数据本身失真或渠道差异），改进生成脚本或向上游反馈。

排查时可以做的两件事：用 **New API: 测试连接** 看每个配置组的延迟与模型数，
以及把扩展日志级别调到 `debug`——网关覆盖了数据表的哪些字段、数值被怎么校正都会记在那里。
...
数据表只是生成时的快照；两者不一致时以网关为准（覆盖与校正的原因写在日志的 debug 级别）。
