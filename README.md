...
- **可解释的模型信息**：悬浮提示里**每个数值都标注来源**（模型数据表 / 网关返回值 / 默认值），
  来源冲突时给出提示。
...
- **状态栏与状态面板**：状态栏显示连接状态与模型数量；面板展示站点信息、模型清单、被过滤的模型、
  会话用量与适配器链。
...
数据表只是生成时的快照。两者显著不一致时 tooltip 会写明「已采用网关值」，
最后还会做一次一致性校正（例如输出上限不能挤占输入空间）并把每次修正记在 tooltip 里。
